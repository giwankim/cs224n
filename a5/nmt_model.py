...
class NMT(nn.Module):
    ...
    def forward(self, source: List[List[str]], target: List[List[str]]) -> torch.Tensor:
        ...
        # Convert lists of sentences into tensors of shape (max_sentence_length, batch_size)
        target_padded = self.vocab.tgt.to_input_tensor(target, device=self.device)

        # Fetch character level tensors
        # tensors have shape (max_sentence_length, batch_size, max_word_length)
        ...
        enc_hiddens, dec_init_state = self.encode(
            source_padded=source_padded_chars,
            source_lengths=source_lengths)
    
        enc_masks = self.generate_sent_masks(enc_hiddens=enc_hiddens, source_lengths=source_lengths)
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[
        ...
        # Lookup character-based word embeddings
        X = self.model_embeddings_source(source_padded)  # (max_sentence_length, b, max_word_length)
        X = pack_padded_sequence(X, source_lengths)

        # Apply the LSTM-encoder; `enc_hiddens` has shape (max_sentence_length, b, h*2)
        # `last_hidden` and `last_cell` have shape (2, b, h)
        enc_hiddens, (last_hidden, last_cell) = self.encoder(X)
        enc_hiddens, sequence_lengths = pad_packed_sequence(enc_hiddens)
        enc_hiddens = enc_hiddens.permute(1, 0, 2)  # (b, max_sentence_length, h*2)

        # Compute decoder initial state

        # Concat the two directions of last hidden states
        last_hidden = torch.cat((last_hidden[0], last_hidden[1]), dim=1)  # (b, h*2)
        # linear projections of last_hidden
        init_decoder_hidden = self.h_projection(last_hidden)  # (h, )

        # Concat the two directions of last cells
    ...
    def decode(self, enc_hiddens: torch.Tensor, enc_masks: torch.Tensor,
        ...
        # Lookup target embedding
        Y = self.model_embeddings_target(target_padded)      # (tgt_len, b, e)

        # Combine `Y_t` with combined-output vector from the previous timestep
        for Y_t in torch.split(Y, 1, dim=0):
            Y_t = Y_t.squeeze(dim=0)                  # (b, e)
            Ybar_t = torch.cat((Y_t, o_prev), dim=1)  # (b, e + h)

            # Compute one step forward including the attention scores
            # `dec_state` has shape (b, h)
            # `o_t` has shape (b, h)
            # `e_t` has shape (b, src_len)
    ...
    def step(self, Ybar_t: torch.Tensor,
        ...
        O_t = F.dropout(torch.tanh(V_t), p=self.dropout_rate)
